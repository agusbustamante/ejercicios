(() => {
  ...
  const obtenerValorOrden = (item, columna) => {
    switch(columna) {
      case 'MesDeLiquidacion':
        const mes = (item.MesDeLiquidacion || '').split(' ')[0];
        return ordenMeses[mes] || 0;
      case 'SueldoBasico':
      case 'Monto_no_remunerativo_1':
        return Number(item[columna]) || 0;
      case 'concepto_no_remunerativo_1':
        return conceptosMap[item.concepto_no_remunerativo_1] || item.concepto_no_remunerativo_1 || '';
      default:
        return item[columna] || '';
    }
  };
  ...
  const renderFiltrado = () => {
    ...
    if (ordenActual.columna) {
      lista.sort((a, b) => {
        const valA = obtenerValorOrden(a, ordenActual.columna);
        const valB = obtenerValorOrden(b, ordenActual.columna);
  ...
  const enviarFormulario = async (ev) => {
    ...
    const url = esAlta ? 'alta.php' : 'modi_debug.php';
