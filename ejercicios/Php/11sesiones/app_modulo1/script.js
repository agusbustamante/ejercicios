(() => {
  ...
  const pasaFiltros = (r) => {
    const f = {
      ...
      apellido : norm(filtros.apellido?.value),
    ...
    if (f.apellido && !norm(r.ApellidoYNombres).includes(f.apellido)) return false;
    ...
    if (f.concepto && !norm(conceptosMap[r.concepto_no_remunerativo_1] || r.concepto_no_remunerativo_1 || '').includes(f.concepto)) return false;
    
    if (f.sueldo) {
      const sueldoNum = parseFloat(f.sueldo);
      const registroSueldo = parseFloat(r.SueldoBasico || 0);
      if (!isNaN(sueldoNum) && registroSueldo !== sueldoNum) return false;
      if (isNaN(sueldoNum)) {
        const sueldoStr = String(r.SueldoBasico || '');
        const sueldoFormateado = formatearNumero(r.SueldoBasico);
        if (!sueldoStr.includes(f.sueldo) && !sueldoFormateado.toLowerCase().includes(f.sueldo.toLowerCase())) return false;
      }
    }
    
    if (f.monto) {
      const montoNum = parseFloat(f.monto);
      const registroMonto = parseFloat(r.Monto_no_remunerativo_1 || 0);
      if (!isNaN(montoNum) && registroMonto !== montoNum) return false;
      if (isNaN(montoNum)) {
        const montoStr = String(r.Monto_no_remunerativo_1 || '');
        const montoFormateado = formatearNumero(r.Monto_no_remunerativo_1);
        if (!montoStr.includes(f.monto) && !montoFormateado.toLowerCase().includes(f.monto.toLowerCase())) return false;
      }
    }
